...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($Funcionario_Nombres) || empty($Funcionario_Apellidos) || empty($Acudiente_Nombres) || empty($Acudiente_Apellidos) || empty($Estudiante_Nombres) || empty($Estudiante_Apellidos)) {
        header('Location: pension.php?error=missing_fields');
        exit; 
    }
    ...
    $miInsert = $miPDO->prepare('INSERT INTO pension (Id_Funcionario, Id_Acudiente, Id_Estudiante, Ciudad, Fecha, Descuento, Monto) 
    VALUES (?, ?, ?, ?, ?, ?, ?)');
...
<form action="" method="post">
    <center><h2>Crear Pensión</h2></center>
    <p>
        <div>Funcionario</div>
